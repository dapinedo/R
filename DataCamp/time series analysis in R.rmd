...
- Mean centered version
  ...
  - $Y_t - \mu = \phi(Y_{t-1} - \mu) + \epsilon_t$ where $\epsilon_t$ is mean zero white noise (WN)
    - The mean $\mu$
    - The slope $\phi$
...
- if $\mu$ = 0 and slope $\phi$ = 1, then:
  - $Y_t = Y_{t-1} + \epsilon_t$
  - Random Walk (RW) and $Y_i$ i **not** stationary in this case
...
In this exercise, you'll use the `acf()` command to estimate the autocorrelation function for three new simulated AR series (`x`, `y`, and `z`). These objects have slope parameters 0.5, 0.9, and -0.75, respectively.
...
Both `x` and the `AirPassengers` data are preloaded in your environment. The time series `x` is shown in the figure below.
...
**predict()** function can be used to make forecasts from an estimated AR model. In the object generated by your `predict()` command, the `$pred` value is the forecast, and the `$se` value is the standard error for the forecast.
...
- The simple moving average (MA) model:
  ...
  - $Y_t = \mu + \epsilon_t + \theta\epsilon_{t-1}$ where $\epsilon_t$ is mean zero white noise (WN)
- Three parameters:
  - The mean $\mu$
  - The slope $\theta$
  - The WN variance $\sigma^2$
- if slope $\theta$ = 0 then it is a white noise (WN) process
- if slope $\theta$ is **not** zero then $Y_t$ depends on both $\epsilon_t$ and $\epsilon_{t-1}$
  - The process $Y_t$ is autocorrelated
- Large values of slope $\theta$ lead to greater autocorrelation
...
In this exercise, you'll use `acf()` to estimate the ACF for three simulated MA series, `x`, `y`, and `z`. These series have slope parameters of 0.4, 0.9, and -0.75, respectively.
...
- sigma^2 = $\hat{\sigma}^2_\epsilon$
- MA fitted values:
  - $\hat{Today} = \hat{Mean} + \hat{Slope} * \hat{Yesterday's\ Noise}$
  - $\hat{Y}_t = \hat{\mu} + \hat{\theta}\hat{\epsilon}_{t-1}$
- Residuals:
  - $Today - \hat{Today}$
  - $\hat{\epsilon}_t = Y_t - \hat{Y}_t$
...
In this exercise, you'll practice using a preloaded time series (`x`, shown in the plot below) as well as the `Nile` dataset used in earlier chapters. 
...
Goodness of fit:
- information criterion (aic and bic) measures in model fitting outputs help to compare which model best fits
  - lower values indicates better fitting model
...
To estimate these indicators, you can use the [AIC()](https://www.rdocumentation.org/packages/stats/versions/3.3.1/topics/AIC) and [BIC()](https://www.rdocumentation.org/packages/stats/versions/3.3.1/topics/AIC) commands, both of which require a single argument to specify the model in question.
...
- the RW and AR models should show strong persistence
- The MA model shows short-run dependence but reverts quickly to the mean
- the WN model does not show any clear patterns.

**Notes on autocorrelation**:

- autocorrelation for the first lag only is consistent with the expectations of the MA model
- autocorrelation across several lags is consistent with the AR model 
- considerable autocorrelation for many lags is consistent with a RW model. 
- no autocorrelation is consistent with a WN model. 

**Congratulations**

- What you've learned
  - manipulating `ts` objects, including `log()` and `diff()`
  - time series models: white noise, random walk, autoregression, simple moving average
  - time series simulation (`arima.sim()`), fitting (`arima`) and forecasting (`predict`)
