...
**Observers** are side effects/actions, not returning values

**Reactive expressions** return values without side effects
...
Observers can be used to explicitly trigger actions in a Shiny app. Later in this chapter, You will learn about variants of the `observe()` function, that provide more control over how actions are triggered
...
This dashboard is pretty awesome, and is definitely extendable. You could add a theme, write a custom CSS stylesheet to add pictures of aliens, and use the data to add even more information about alien sightings the world over.
...
For the extra challenge, you can use `forcats::fct_reorder()` to order the ingredients in descending order of `tf_idf`.
...
There is a rich ecosystem of interactive widgets like d3wordcloud, that make it easy to add interactivity to your Shiny app. Look up the gallery of htmlwidgets at [http://gallery.htmlwidgets.org/](http://gallery.htmlwidgets.org/)
...
Mass Shootings have been a topic of intense discussion in the United States. A public database of mass shootings since 1982 has been made available by [Mother Jones](https://www.motherjones.com/politics/2012/12/mass-shootings-mother-jones-full-data/), a non-profit organization. Over the next three exercises, you will build a Shiny app to explore these shootings on an interactive map.
